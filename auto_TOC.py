...
with io.open(os.path.join(qmrlab_dir,'src/tmp/tmpDocDir.txt'),'r') as fpwdTemp:
	pwd = (fpwdTemp.readline()).rstrip('\n')
fpwdTemp.close()

for root, dirs, files in os.walk(os.path.join(qmrlab_dir,'src/Models')):
			...
			for root_s, dirs_s, files_s in os.walk(pwd):
				for name_s in files_s:
					...
					if name_s.endswith(".html") and cfile.lower() == name_s[:name_pos].lower():
						...
						#pwd is the temporary dir where all the files are saved
						...
						os.system("python3 embed_html.py "+dst+" "+src+" \""+cname+"\"")
					#Copy the png files
